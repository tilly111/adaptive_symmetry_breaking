...
/* current commitment */
uint8_t my_commitment=1;  // TODO change here
...
// model parameter -> offset on how much better the discovered site needs to be then the current one while comparing
float param =0.1;
...
double AngleToGoal(){
    ...
    double angletogoal=atan2(Goal_GPS_Y-Robot_GPS_Y,Goal_GPS_X-Robot_GPS_X)/PI*180-Robot_orientation;  // TODO shadowing? why do we dont have to pass parameter to this function -> gloabl assigned?
...
void set_motion( motion_t new_motion_type ) {
    ...
    if( current_motion_type != new_motion_type ){
        ...
        int calibrated = true;  // flag if motor is calibrated
        ...
        if(current_motion_type!=STOP){
            backup_motion=current_motion_type;  // TODO set back up motion to new motion if not STOP?
        }
    }
}
...
void message_rx( message_t *msg, distance_measurement_t *d ) {
    ...
    /** ARK Runtime identification **/
        ...
        printf("RUNTIME IDENTIFICATION MSG");
    ...
    else if (msg->type == 7) {  // is never used
        printf("IS ACTUALLY IN USE");
        redBots = msg->data[0];
    ...
    /** Message from another robot **/
        ...
        printf("SOME WRONG MESSAGES HAS BEEN SEND");
    }
}
...
void sample_option_quality() {
    ...
    if( ( discovered_option_GPS_X == my_option_GPS_X ) && ( discovered_option_GPS_Y == my_option_GPS_Y ) ){
        ...
        if(GoingToResampleOption) {
            GoingToResampleOption=false;
            random_walk_waypoint_model(SELECT_NEW_WAY_POINT); // after resampling the robot resume random walk
        }
        discovered = false;
...
void update_commitment() {
    ...
    if( kilo_ticks > last_update_ticks + update_ticks ) {
        ...
        /* if the agent is committed */
            ...
            // if discovered site is better we set it with prob depending on the sites quality
            if (discovered){
                // 1 if better option / 0 else -> make sure to only update with better option by param
            ...
            /* Direct-switch */
            // received a message -> not no site and different from my site
...
/*-------------------------------------------------------------------*/
void broadcast() {
    // if enough time has passed and the kilobot wants to broadcase
...
void check_if_against_a_wall() {
    if ( Robot_GPS_X==0 || Robot_GPS_X>=GPS_maxcell-1 || Robot_GPS_Y==0 || Robot_GPS_Y>=GPS_maxcell-1 ){
        ...
        else wallAvoidanceCounter = 1; // does this makes sense or should we do it to 18
    } else {
...
void loop() {
    if(!runtime_identification){
        ...
        if (new_sa_msg_global_comm == true) { // is never called
            printf("IS CALLED ERROR TIME!!!!! -> hard coded populations!");
        ...
        if (number_of_options==5){
            ...
            case 0:
                ...
                break;
            default:
                    printf("SHOULDNT HAPPEN SOMETHINGS WRONG");
                set_color(RGB(3,3,3));
                break;
            }
        }
