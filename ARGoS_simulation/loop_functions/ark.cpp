...
CArk::CArk(): CLoopFunctions(),m_fTimeInSeconds(0),m_unMessagingCounter(0), m_unDataSavingCounter(1),m_bDynamicVirtualEnvironments(false),m_unDataFrequency(100),m_unOhcFrequency(10),
m_fQuorum(1.0),m_bQuorumReached(false),m_bOptionsLookUpTableSent(false),initial_opinion(20),m_unOptionsSent(-1){} // m_unEnvironmentPlotUpdateFrequency(10)
CArk::~CArk(){
}
...
/****************************************/
// some notes:
// m_fQuorum - varaible can be set (0.8)
// m_fQuorumRobots - mfQuorum * number of robots
void CArk::PostStep(){
    /* Save experiment data to the specified log file*/
    std::fill(m_tCommitmentState.begin(), m_tCommitmentState.end(), 0);  // sets the commitment state to 0 for all robots
    for(unsigned int i=0;i< m_tKilobotsEntities.size();i++){ // for every robot
        m_tCommitmentState[((unsigned int) m_tKBs[i]->commitement)]++;
        //printf("%d %d %d %d \n", m_tCommitmentState[0], m_tCommitmentState[1], m_tCommitmentState[2], m_tCommitmentState[3]);
        if((m_tKBs[i]->commitement!=initial_opinion) && (m_tKBs[i]->commitement!=0) && (m_fQuorumRobots>0) && (m_tCommitmentState[((unsigned int) m_tKBs[i]->commitement)]>=m_fQuorumRobots) ){
...
// just returns the x,y position of the kilobot
...
// returns the orientation -> angle around z axis
...
void CArk::SetupVirtualEnvironments(TConfigurationNode& t_tree){
    ...
    if(type=="manual"){
        ...
        std::vector<int> posShuffling={1,2,3,4,5};  // TODO change back to 3
...
void CArk::GetExperimentVariables(TConfigurationNode& t_tree){
    ...
    GetNodeAttributeOrDefault(tExperimentVariablesNode, "initial_opinion", initial_opinion, initial_opinion);
...
// TODO is this the called method by argos? -> sets if updates are required
...
/****************************************/
/****************************************/
// for every kilobot in the simulation
void CArk::UpdateKilobotState(CKilobotEntity &c_kilobot_entity){
    ...
    /* Check if the robot encountered an option */
    ...
    // in case the robot encounters more than one option -> should this be possible ?
...
void CArk::UpdateVirtualSensors(){
    ...
    // set to true after sending the inital lookup table
    if(m_unOptionsSent == (SInt16)m_tOptions.size()){
...
void CArk::UpdateVirtualSensor(CKilobotEntity &c_kilobot_entity){
    ...
    if(!m_bOptionsLookUpTableSent){
        ...
        /* Discovery Messages*/
        // send if after certain interval
        ...
        {
            // if the option side exists basically apperance < current time < disaperance || disaperance < 0 what means that it never dissapears?!?
